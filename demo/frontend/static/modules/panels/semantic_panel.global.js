    ...
    function renderSemanticPanel(payload, semanticState) {
      ...
      elements.semanticPanel.innerHTML = `
        ...
        <div class="title-with-help section-title-with-help">
          <h3>Local Semantic Fields for Line ${payload.line_number}${payload.line_text ? `: <span class="semantic-title-inline">${escapeHtml(payload.line_text)}</span>` : ""}</h3>
        ...
        button.addEventListener("click", () => {
          ...
          if (state.activeSemanticField) {
            requestAnimationFrame(() => {
              const target = elements.recordsList?.querySelector(".record-card") || elements.recordsSection;
